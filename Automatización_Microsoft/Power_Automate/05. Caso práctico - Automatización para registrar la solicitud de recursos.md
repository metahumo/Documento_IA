...
# Automatización para registrar la solicitud de talleres — Documentación técnica (versión censurada)
...
Antes de activar el flujo, prepara estas dependencias. Los nombres de carpetas/listas deben ser consistentes con los usados en el flujo.

### A) Listas en SharePoint (sitio de trabajo)

- **Sitio:** [REDACTADO] (ejemplo orientativo: `https://<tu_org>.sharepoint.com/sites/<tu_sitio>`) ✅ CENSURADO

1) **EntidadesTalleres** (lista de referencia)
- Columnas sugeridas:
  - **Title** (Texto) — nombre de la entidad
  - **EMAIL** (Texto) — correo del remitente
  - **PALABRACLAVE** (Texto) — palabra clave asociada (p. ej. "taller")
...
4. Tras crear el registro, comprobar si el remitente existe ya en `EntidadesTalleres` (solo si hubo detección):
...
**Configuración:**
- **Dataset/Sitio:** [REDACTADO] ✅ CENSURADO
- **Tabla (lista):** `EntidadesTalleres` (Id interna: [REDACTADO]) ✅ CENSURADO
- **Filtro:** `ACTIVO eq 1`
...
- Expresión:
  - `contains(toLower(subject), toLower(coalesce(item.PALABRACLAVE,'')))` OR
  - `contains(toLower(body), toLower(coalesce(item.PALABRACLAVE,'')))`
- SI:
...
- **Mover_correo_electrónico_(V2)** — Outlook → MoveV2
  ...
  - **Carpeta:** `Talleres` (id de carpeta resuelto vía metadata)

- **Crear_elemento** — SharePoint → PostItem (lista `SolicitudesTalleres`)
  - **Dataset:** [REDACTADO] ✅ CENSURADO
  - **Tabla (lista):** Id interna [REDACTADO] ✅ CENSURADO
  - **Campos:**
...
- A continuación, dentro de esta misma condición:
  - **Obtener_elementos** — SharePoint → GetItems (lista `EntidadesTalleres`)
    - **FolderPath:** [REDACTADO] (p. ej., `/Lists/Talleres`) ✅ CENSURADO
...
Se ejecuta dentro de `Condición_global`, tras crear el elemento en `SolicitudesTalleres`.
...
- **Falsos positivos**: palabras clave genéricas (p. ej., "taller") pueden generar detecciones no deseadas si aparecen en comunicaciones no relacionadas.
